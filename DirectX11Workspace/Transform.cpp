...
void Transform::UpdateTransform()
{
	...
	if (HasParent())
	{
		_worldMat = _localMat * _parent->GetWorldmatrix(); // _worldMat는 말 그대로 나로부터 world까지 변하는 변환 행렬이다.
...
void Transform::SetRotation(const Quaternion& worldRotation)
{
	if (HasParent())
	{
		Quaternion invParentRot = XMQuaternionInverse(_parent->GetRotation()); // 쿼터니언은 날먹이 된다.
