...
// 이건 뭔가요? -> enum class는 int로 명시적으로 치환되지 않아, 연산자를 지원하지 않는다. 하지만 그러면 일일이 캐스팅을 해줘야하는 불편함이 남는다.
...
public:
	...
	const Microsoft::WRL::ComPtr<ID3DBlob>& GetBlob() const { return _blob; }
...
protected:
	...
	Microsoft::WRL::ComPtr<ID3DBlob> _blob = nullptr; // 셰이더 로드, 저장
};
...
public:
	VertexShader(Microsoft::WRL::ComPtr<ID3D11Device> device);
	~VertexShader() override;

	const Microsoft::WRL::ComPtr<ID3D11VertexShader>& GetComPtr() const  { return _vertexShader; }
...
class PixelShader : public Shader
{
	using Super = Shader;
public:
	PixelShader(Microsoft::WRL::ComPtr<ID3D11Device> device);
	~PixelShader() override;

	const Microsoft::WRL::ComPtr<ID3D11PixelShader>& GetComPtr() const { return _pixelShader; }
